...
const page = ({ params }) => {
    ...
    const getData = async () => {
      ...
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_URL}/cmts/${id}`
        );
        setCmtData(res.data);
  ...
  const onSubmit = async (data) => {
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_URL}/cmt`, data);
  ...
  return (
    <>
      <div className=" w-full flex flex-row justify-center h-auto ">
        <div className=" w-full sm:w-1/2  ">
          ...
          <div className="mt-4">
            {!(cmtArr == [] || cmtData) && (
              <div> Nothing here, Be the first one to Comment!</div>
            )}
